...
const SEED_B64: &str = "d38T7zjbGpQh1uat84ZP5B6OfdHoqc/6nNafXwe4Tb8=";
...
fn valid_payload() -> PairingPayload {
    PairingPayload {
        ...
        ephemeral_private_key: SEED_B64.into(),
    }
}

fn decompress(encoded: &[u8]) -> Vec<u8> {
    let mut decoder = ZlibDecoder::new(encoded);
    let mut json = Vec::new();
    decoder
        .read_to_end(&mut json)
        .expect("encoded payload decompresses");
    json
}
...
fn qr_payload_round_trips_every_required_pairing_field() {
    ...
    let json = decompress(&encoded);
    assert_eq!(serde_json::from_slice::<PairingPayload>(&json).unwrap(), payload);
}

#[test]
fn qr_payload_is_compressed_bytes_not_plain_json_or_url() {
    let encoded = valid_payload().encode_for_qr().expect("payload encodes");
    let as_text = String::from_utf8_lossy(&encoded);

    assert!(!as_text.contains("timu://"));
    assert!(!as_text.contains("ephemeral_private_key"));
    assert!(!as_text.contains(SEED_B64));
}

#[test]
fn qr_payload_rejects_uncompressed_json() {
    let json = serde_json::to_vec(&valid_payload()).expect("payload serializes");
    assert_eq!(
        PairingPayload::decode_from_qr(&json, 1_700_000_000),
...
fn qr_payload_rejects_expired_credentials() {
    let payload = valid_payload();
    let encoded = payload.encode_for_qr().expect("payload encodes");
...
fn qr_payload_rejects_unknown_json_fields() {
    ...
    encoder.write_all(json.as_bytes()).expect("compresses");
    let encoded = encoder.finish().expect("compresses");
    assert_eq!(
...
#[test]
fn ephemeral_private_key_must_be_base64_ed25519_seed() {
    let mut not_base64 = valid_payload();
    not_base64.ephemeral_private_key = "-----BEGIN OPENSSH PRIVATE KEY-----".into();
    let not_base64 = not_base64.encode_for_qr().expect("payload encodes");
    assert_eq!(
        PairingPayload::decode_from_qr(&not_base64, 1_700_000_000),
        Err(PayloadError::Invalid)
    );

    let mut wrong_length = valid_payload();
    wrong_length.ephemeral_private_key = STANDARD.encode(b"not-a-32-byte-seed");
    let wrong_length = wrong_length.encode_for_qr().expect("payload encodes");
    assert_eq!(
        PairingPayload::decode_from_qr(&wrong_length, 1_700_000_000),
        Err(PayloadError::Invalid)
    );
}
